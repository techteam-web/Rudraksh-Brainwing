...
const HomePage = ({ onExplore }) => {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      const loadingTl = gsap.timeline({ onComplete: () => startRevealAnimation() });
      gsap.to({}, {
        duration: 2,
        onUpdate: function () {
      ...
      loadingTl
        .to(loadingTextRef.current, { opacity: 1, y: 0, duration: 0.6, ease: "power3.out" })
        .to(loadingProgressRef.current, { scaleX: 1, duration: 2, ease: "power2.inOut" }, 0.2)
        .to(loadingTextRef.current, { opacity: 0, y: -30, duration: 0.4 }, 2)
        .to(loadingRef.current, { clipPath: "polygon(0 0, 100% 0, 100% 0, 0 0)", duration: 1, ease: "power4.inOut" }, 2.2);
    }, containerRef);
    return () => ctx.revert();
  }, []);
  ...
  const startContinuousAnimations = () => {
    gsap.to(buttonGlowRef.current, { scale: 1.5, opacity: 0.3, duration: 2, yoyo: true, repeat: -1, ease: "sine.inOut" });
    gsap.to(glowOrbRef.current, { scale: 1.2, opacity: 0.3, duration: 4, yoyo: true, repeat: -1, ease: "sine.inOut" });
    
    orbsRef.current.forEach((orb, i) => {
      if (orb) gsap.to(orb, { y: "+=" + (20 + i * 10), rotation: 360, duration: 12 + i * 3, yoyo: true, repeat: -1, ease: "sine.inOut" });
    });
    ...
    particlesRef.current.forEach((particle) => {
      if (particle) {
        ...
        gsap.to(particle, {
          ...
          repeat: -1,
          ease: "none",
          onRepeat: () => gsap.set(particle, { y: window.innerHeight + 50, x: Math.random() * window.innerWidth }),
        });
        gsap.to(particle, { opacity: Math.random() * 0.4 + 0.1, duration: 3 + Math.random() * 2, yoyo: true, repeat: -1, ease: "sine.inOut" });
      }
    });
  };
  ...
  return (
    <div ref={containerRef} className="h-screen max-h-screen relative overflow-hidden" style={{ background: colors.bg, cursor: "none" }}>
      <style>{`
      ...
      <div ref={cursorRef} className="fixed w-12 h-12 rounded-full hidden md:block" style={{ zIndex: 9999, border: `2px solid ${colors.textPrimary}`, pointerEvents: "none", boxShadow: `0 0 20px ${colors.glowPrimary}`, top: 0, left: 0 }} />
      <div ref={cursorDotRef} className="fixed w-3 h-3 rounded-full hidden md:block" style={{ zIndex: 9999, pointerEvents: "none", backgroundColor: colors.textPrimary, boxShadow: `0 0 10px ${colors.textPrimary}`, top: 0, left: 0 }} />

      {/* Loading Screen */}
      <div ref={loadingRef} className="fixed inset-0 flex flex-col items-center justify-center" style={{ zIndex: 100, background: colors.bg, clipPath: "polygon(0 0, 100% 0, 100% 100%, 0 100%)" }}>
        <div className="relative">
          <div ref={loadingTextRef} className="text-2xl sm:text-3xl md:text-5xl font-light tracking-[0.4em] sm:tracking-[0.6em] uppercase mb-6 sm:mb-8" style={{ fontFamily: "'Cinzel', serif", color: colors.textPrimary, opacity: 0, transform: "translateY(20px)" }}>
            Rudraksh
          </div>
          <div className="relative w-48 sm:w-64 h-1 rounded-full overflow-hidden" style={{ backgroundColor: "rgba(245, 240, 235, 0.2)" }}>
            <div ref={loadingProgressRef} className="absolute inset-y-0 left-0 w-full rounded-full origin-left" style={{ transform: "scaleX(0)", background: `linear-gradient(90deg, ${colors.textSecondary}, ${colors.textPrimary}, ${colors.textAccent}, ${colors.textPrimary})`, boxShadow: `0 0 20px ${colors.textPrimary}` }} />
          </div>
          <div className="text-center mt-3 sm:mt-4 text-xs sm:text-sm tracking-[0.3em]" style={{ color: colors.textSecondary, fontFamily: "'Marcellus', serif" }}>
            {loadingPercent}%
          </div>
        </div>
      </div>
        ...
        <header className="flex-shrink-0 flex justify-between items-center px-4 sm:px-8 md:px-16 py-2 sm:py-3 md:py-4">
          <div ref={logoRef} className="flex items-center gap-2 sm:gap-3 md:gap-4" style={{ opacity: 0 }}>
            ...
            <div className="flex flex-col">
              <span className="text-sm sm:text-base md:text-lg lg:text-xl font-medium uppercase tracking-[0.2em] sm:tracking-[0.3em]" style={{ fontFamily: "'Cinzel', serif", color: colors.textPrimary }}>
                Rudraksh
              </span>
              <span className="text-[7px] sm:text-[8px] md:text-[9px] lg:text-[10px] uppercase tracking-[0.3em] sm:tracking-[0.4em]" style={{ fontFamily: "'Marcellus', serif", color: colors.textSecondary }}>
                Apartments
              </span>
            </div>
          </div>
          ...
          >
            ...
            <div className="relative px-6 sm:px-10 md:px-16 lg:px-24 py-6 sm:py-8 md:py-10 lg:py-12 text-center">
              ...
              <div className="flex justify-center mb-3 sm:mb-4">
                <svg viewBox="0 0 150 40" fill="none" className="w-24 h-6 sm:w-32 sm:h-8 md:w-36 md:h-9">
              ...
              {/* Rudraksh Icon */}
              <div className="flex justify-center mb-3 sm:mb-4">
                <svg viewBox="0 0 56 56" fill="none" className="w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12">
                  <path d="M28 2C28 2 14 12 14 28C14 44 28 54 28 54" stroke={colors.textPrimary} strokeWidth="1.5" strokeLinecap="round" fill="none" />
                  <path d="M28 2C28 2 42 12 42 28C42 44 28 54 28 54" stroke={colors.textPrimary} strokeWidth="1.5" strokeLinecap="round" fill="none" />
                  <circle cx="28" cy="2" r="2.5" fill={colors.textPrimary} />
                  <circle cx="28" cy="54" r="2.5" fill={colors.textPrimary} />
                  <circle cx="28" cy="28" r="4" fill={colors.textAccent} opacity="0.9" />
                </svg>
              </div>

              {/* Main Title */}
              <h1 ref={heroTextRef} className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-light mb-2 sm:mb-3" style={{ fontFamily: "'Cinzel', serif", letterSpacing: "0.08em", lineHeight: 1.1, color: colors.textPrimary, opacity: 0 }}>
                RUDRAKSH
              </h1>

              {/* Apartments Subtitle */}
              <div ref={heroSubRef} className="flex items-center justify-center gap-3 sm:gap-4 md:gap-6 mb-2 sm:mb-3" style={{ opacity: 0 }}>
                <span className="w-10 sm:w-14 md:w-20 h-px" style={{ background: `linear-gradient(to right, transparent, ${colors.textPrimary})` }} />
                <span className="text-xs sm:text-sm md:text-base lg:text-lg uppercase tracking-[0.25em] sm:tracking-[0.3em] md:tracking-[0.4em]" style={{ fontFamily: "'Marcellus', serif", color: colors.textSecondary }}>
                  Apartments
                </span>
                <span className="w-10 sm:w-14 md:w-20 h-px" style={{ background: `linear-gradient(to left, transparent, ${colors.textPrimary})` }} />
              </div>

              {/* Tagline */}
              <p ref={taglineRef} className="text-sm sm:text-base md:text-lg lg:text-xl italic mb-4 sm:mb-5 md:mb-6" style={{ fontFamily: "'Cormorant Garamond', serif", color: colors.textPrimary, fontWeight: 300, opacity: 0 }}>
              ...
              <div className="flex justify-center gap-8 sm:gap-12 md:gap-20 lg:gap-28 mb-4 sm:mb-5 md:mb-6">
                {[
                  ...
                  <div key={i} className="text-center">
                    <div className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-light" style={{ fontFamily: "'Cinzel', serif", color: colors.textPrimary }}>
                      {stat.value}
                    </div>
                    <div className="text-[8px] sm:text-[9px] md:text-[10px] uppercase tracking-[0.2em] sm:tracking-[0.3em] mt-1" style={{ fontFamily: "'Marcellus', serif", color: colors.textSecondary }}>
                      {stat.label}
                    </div>
                  </div>
                ))}
              ...
              <div className="relative inline-block">
                ...
                <button
                  ...
                  style={{ background: "rgba(245, 240, 235, 0.15)", border: "1px solid rgba(245, 240, 235, 0.3)", backdropFilter: "blur(10px)", boxShadow: "0 10px 40px rgba(0, 0, 0, 0.1)", opacity: 0, cursor: "pointer" }}
                >
                  <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500" style={{ background: "linear-gradient(105deg, transparent 40%, rgba(255,255,255,0.15) 50%, transparent 60%)" }} />
                  <span className="text-xs sm:text-sm md:text-base uppercase tracking-[0.2em] sm:tracking-[0.3em] relative whitespace-nowrap" style={{ fontFamily: "'Marcellus', serif", color: colors.textPrimary }}>
                    Explore the Home
                  </span>
                </button>
              </div>
              ...
              <div className="flex justify-center mt-4 sm:mt-5 md:mt-6">
                <svg viewBox="0 0 150 30" fill="none" className="w-28 h-6 sm:w-36 sm:h-7 md:w-40 md:h-8">
        ...
        <footer className="flex-shrink-0 flex items-center px-4 sm:px-8 md:px-16 py-2 sm:py-3 md:py-4">
          <div ref={soundControlsRef} className="flex items-center gap-2 sm:gap-3" style={{ opacity: 0 }}>
            <button
              ...
              style={{ backgroundColor: "rgba(245, 240, 235, 0.1)", border: "1px solid rgba(245, 240, 235, 0.2)", backdropFilter: "blur(10px)" }}
            >
              <div className="absolute inset-0 rounded-full opacity-0 group-hover:opacity-100 transition-opacity" style={{ background: `radial-gradient(circle, ${colors.glowPrimary} 0%, transparent 70%)` }} />
              {isMuted ? (
                <svg viewBox="0 0 24 24" fill="none" className="w-3.5 h-3.5 sm:w-4 sm:h-4 relative" stroke={colors.textSecondary} strokeWidth="1.5">
            ...
            <span className="text-[7px] sm:text-[8px] md:text-[9px] uppercase tracking-[0.15em] sm:tracking-[0.2em]" style={{ fontFamily: "'Marcellus', serif", color: colors.textSecondary }}>
